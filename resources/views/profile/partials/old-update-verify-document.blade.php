<section class="flex flex-col justify-center">
    ...
    Update your govt. Issued ID,Business CAC Document
    </p>
    </header>
    
    <div class="grid grid-cols-1 lg:grid-cols-2 w-full  gap-y-4 lg:gap-x-10">
        @if ($user->govt_id)
    ...
    {{-- <form method="post" >  
        action="{{ route('destroy.govtid') }}" class=""
    @csrf
    @method('delete')
    ...
    <input type="file" accept="image/*"   @change="updatePreview" x-ref="fileInput"
    ...
    <form action="{{ route('upload.documents') }}" method="POST" enctype="multipart/form-data">
      ...
      <button class="bg-ageno-2 text-white h-[40px] px-4 rounded-md">Update</button>
