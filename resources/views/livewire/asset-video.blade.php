...
<img src="{{ Storage::url($user->cac_document) }}" alt="Government ID" class="w-full  h-[200px] rounded-lg object-cover">
</div> 

<div class="mt-2 flex gap-x-10">
<form method="post" action="{{ route('destroy.cac_document') }}" class="">
@csrf
@method('delete')
...
<p class="dark:text-neutral-300">Are you sure want to delete your CAC Document? </p>
<div class="mt-2">
<div class="font-bold">Note:</div>  
<ul class="list-decimal">
<li>
<span class="italic text-sm">
Your CAC Document is required for verification
</span></li>
<li>
<span class="italic text-sm">
Your CAC Document will be removed from our database storage 
</span></li>
</ul>
...
<button type="button" @click="previewCacModal = true"  class="group bg-ageno-2 text-white px-4 py-2 rounded outline-none border-none ring-0 flex items-center gap-x-1">
  Preview
  ...
  <template x-if="true">
    <div  x-show="previewCacModal" 
    class="fixed inset-0 flex items-center justify-center z-[1000]">
    <div class="fixed inset-0 bg-black opacity-50" @click="previewCacModal = false"></div>
    <div x-show="previewCacModal"
    x-transition:enter="transition ease-out duration-300 transform" 
    x-transition:enter-start="scale-95 opacity-0" 
    x-transition:enter-end="scale-100 opacity-100" 
    ...
    <button type="button" @click="previewCacModal = false" 
    ...
    <div class="flex flex-col justify-between shadow-sm p-2 w-full min-w-full overflow-x-hidden overflow-y-auto h-screen">
      <img src="{{ Storage::url($user->cac_document) }}" alt="CAC Document" class="w-full h-full rounded-2xl object-contain">
    </div>
    </div>
    </div>
  </template>
...
<img class="size-28 max-w-full rounded-lg" :src="previewUrl" alt="User CAC Document">
...
<span class="text-blue-500">Click to upload your CAC Document</span>
or drag and drop
</p>
